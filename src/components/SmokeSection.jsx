...
const SmokeEffect = () => {
  ...
  useEffect(() => {
    ...
    // Ensure the canvas size is set based on its clientWidth/clientHeight
  ...
  useEffect(() => {
    ...
    const centerX = canvasWidth / 2; // Center X coordinate of the canvas
    const centerY = canvasHeight / 2; // Center Y coordinate of the canvas
    let startX = 0; // Initial X position of the line (start from left)
    let startY = centerY; // Initial Y position of the line (center of viewport)
    let targetX = canvasWidth; // Initial target X position (rightmost)
    let targetY = centerY; // Initial target Y position (center of viewport)
    const speed = window.innerWidth < 768 ? 5 : 20; 
  
    if (isAnimating) {
      console.log('Line animation started!');
      // Initial movement from left to right at the center of the viewport
      intervalId = setInterval(() => {
        ...
        if (distance > speed && firstAnimation) {
          startX += (dx / distance) * speed;
          startY += (dy / distance) * speed;
          setFirstAnimation(false);
        } else{
          startX = centerX; // Set X position to the center of the canvas
          targetX = Math.random() * canvasWidth; // Set new random target X position
          targetY = Math.random() * canvasHeight; // Set new random target Y position
          clearInterval(intervalId); // Stop initial movement after reaching the center
          // Continue with the existing code for casual movement
          intervalId = setInterval(() => {
            const dx = targetX - startX;
            const dy = targetY - startY;
            const distance = Math.sqrt(dx * dx + dy * dy);
            ...
            const fakeMouseMoveEvent = new MouseEvent('mousemove', {
              clientX: startX,
              clientY: startY,
            });
            window.dispatchEvent(fakeMouseMoveEvent);
            canvasRef.current.dispatchEvent(fakeMouseMoveEvent);
          }, window.innerWidth < 768 ? 30 : 25);
        }
        const fakeMouseMoveEvent = new MouseEvent('mousemove', {
          clientX: startX,
          clientY: startY,
  ...
  function detectMouseIdle(timeoutDuration = 2000) {
    let timeoutId;
  
    const resetTimer = () => {
      clearTimeout(timeoutId);
      timeoutId = setTimeout(() => {
        // Mouse is considered idle
    ...
    // Return a function to clean up event listener and timer
    return () => {
